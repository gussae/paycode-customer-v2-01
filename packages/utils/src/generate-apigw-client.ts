...
function generateTempPaths(workspaceRoot: string): TempPaths {
  const TEMP_DIR_NAME = '.temp-generateClient';
...
export async function generateApigwClient({
  ...
  try {
    console.log(tempPaths.TEMP_DIR_PATH);
    ...
    //output
    fs.ensureDir(outdir);
    await fs.copy(tempPaths.TEMP_API_TS_CLIENT_DIR, outdir, {
      overwrite: true,
    });
  } catch (error) {
    ...
    // fs.remove(tempPaths.TEMP_DIR_PATH);
  }
}
